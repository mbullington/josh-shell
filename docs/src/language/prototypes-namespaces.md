...
| `Array` | conversion `Array(x)`; `Array.prototype`: `at`, `contains`, `map`, `filter`, `reduce`, `flat`, `join`, `slice`, plus the non-mutating transforms `push`/`pop`/`reverse`/`sort`/`length` that return new arrays and leave the receiver unchanged |
...
- `push`/`pop`/`reverse` return whole new arrays (never elements), `sort` needs all numbers or all strings, and `env.PATH = env.PATH.push("/new/dir")` is the documented way to extend PATH.
