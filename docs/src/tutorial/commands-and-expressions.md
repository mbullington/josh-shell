...
Expression mode evaluates Null, booleans, numbers, strings, arrays, objects, functions, member/index access, calls, spread, ternary expressions, unary `!` and `-`, arithmetic, ordering, strict equality, and short-circuit `&&`/`||`. Expression `&&` and `||` return one operand according to [truthiness](../reference/operators-values.md#J-EXPR-005).
