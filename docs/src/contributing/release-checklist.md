...
**Status coverage:** This page makes no product-availability claims. Passing items provide evidence for specific matrix rows.
...
9. **Manual:** run the checker and persistent mdBook build; inspect generated links/fragments, literal fences, duplicate IDs, code blocks, wait-table shape, status drift, status colors, index/matrix/schema pages, and the retained PNG.
...
test -f docs/book/status/matrix.html
